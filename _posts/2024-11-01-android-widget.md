...
class ExampleAppWidgetProvider : AppWidgetProvider() {
    ...
    // 또한 앱 위젯이 추가 될 떄에도 호출 되므로 Service 와의 상호작용 등의 초기 설정이 필요 할 경우에도 이 메소드를 통해 구현해요.
    override fun onUpdate(
        ...
        appWidgetIds.forEach { appWidgetId ->
            ...
            // AppWidgetManager에게 현재 위젯을 업데이트하라고 말해줘요.
    ...
    // 위젯 ID 는 UID 별로 관리 되는데 이때 복원 시점에서 ID 가 변경 될 수 있으므로 백업 시점의 oldID 와 복원 후의 newID 를 전달하게 돼요.
    ...
    // 해당 앱 위젯이 삭제 될 때 호출 돼요.
    override fun onDeleted(context: Context, appWidgetIds: IntArray) {
        super.onDeleted(context, appWidgetIds)
    }

    // 앱의 브로드캐스트를 수신하며 해당 메서드를 통해 각 브로드캐스트에 맞게 메서드를 호출합니다.
