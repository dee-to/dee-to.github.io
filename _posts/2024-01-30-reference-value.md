...
### 프로그래머로서 생각해야하는 레퍼런스가 무엇인가는 이게 끝이에요.
...
# 가능하면 레퍼런스를 사용하고, 필요할 때는 포인터를 사용하세요.
<br><br><br>
...
> 참조는 선언되는 순간 초기화되어야 하고, 이는 역참조된 nullptr가 아닌 값이어야 해요.
...
### 1. 전달된 객체를 변경하려면 참조로 호출하거나 포인터를 사용하세요.<br>
### 2. 전달된 객체를 변경하고 싶지 않고 *`크기가 큰 경우`* const 참조로 호출하세요.
### 3. 그렇지 않으면 값으로 호출하세요.<br>
<br><br>
